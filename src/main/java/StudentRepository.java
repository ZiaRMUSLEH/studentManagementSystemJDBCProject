...
//class which will be connected with database (connection, statement, prepared statement)
...
public class StudentRepository {
    ...
    private void getPreparedStatement(String query){
        try {this.prs= con.prepareStatement(query);
        }catch (SQLException e){
            System.out.println(e.getMessage());
        }
    }
    ...
    //step -10. method to persist data in database
    ...
    public Student findStudentById (int id) {
        ...
        try {
            getPreparedStatement(query);
            prs.setInt(1,id);
            //data coming from table is ResultSet
    ...
    //step-16 method that will run q
